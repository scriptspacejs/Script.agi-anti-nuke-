...
function initServerData(guildId) {
    if (!serverData.has(guildId)) {
        serverData.set(guildId, {
            ...
            whitelistedBotRoles: new Set() // Store whitelisted bot role IDs
        });
    }
...
// Check if a bot has a whitelisted bot role
function hasWhitelistedBotRole(member, data) {
    if (!member.user.bot || !data || !data.whitelistedBotRoles || data.whitelistedBotRoles.size === 0) return false;
    return member.roles.cache.some(role => data.whitelistedBotRoles.has(role.id));
}
...
function createUnifiedWidget(guild) {
    ...
    const row2 = new ActionRowBuilder()
        .addComponents(
            new ButtonBuilder().setCustomId('whitelist_this_bot').setLabel('Whitelist This Bot').setStyle(ButtonStyle.Success).setEmoji('🤖')
        );
...
// Bot nickname rotation states (without emojis)
const nicknameStates = ['secured', 'armed', 'by script'];
let currentNicknameIndex = 0;

// Function to update bot nickname in all guilds
async function updateBotNicknames() {
    const statusSuffix = nicknameStates[currentNicknameIndex];
    const nextStatusSuffix = nicknameStates[(currentNicknameIndex + 1) % nicknameStates.length];
    
    console.log(`🔄 [GLOBAL] Rotating nickname from index ${currentNicknameIndex} to: "${statusSuffix}"`);
    ...
    for (const [guildId, guild] of client.guilds.cache) {
        try {
            ...
            if (!me) {
                console.log(`⚠️ [${guild.name}] Bot member not found in cache`);
                failCount++;
                continue;
            }
            
            // Skip if bot is the guild owner (can't change own nickname as owner)
            if (me.id === guild.ownerId) {
                console.log(`⚠️ [${guild.name}] Bot is server owner - cannot change own nickname`);
                permissionIssues++;
                continue;
            }
            
            // Check role hierarchy - bot needs a role higher than @everyone
            const botHighestRole = me.roles.highest;
            const everyoneRole = guild.roles.everyone;
            
            if (botHighestRole.position <= everyoneRole.position) {
                console.log(`⚠️ [${guild.name}] Bot has no elevated roles - move bot role higher in Server Settings → Roles`);
                permissionIssues++;
                continue;
            }
            
            // Check if bot has Change Nickname permission
            const hasPermission = me.permissions.has(PermissionFlagsBits.ChangeNickname);
            
            if (!hasPermission) {
                console.log(`⚠️ [${guild.name}] Missing "Change Nickname" permission - enable in role settings`);
                permissionIssues++;
                continue;
            }
            
            // Build fixed nickname: ".gg/scriptspace.in | secured"
            const nickname = `.gg/scriptspace.in | ${statusSuffix}`;
            
            // Discord nickname limit is 32 characters, truncate if needed
            const finalNickname = nickname.length > 32 ? nickname.substring(0, 32) : nickname;
            
            // Attempt to set nickname with retry logic
            let retryCount = 0;
            let success = false;
            
            while (retryCount < 3 && !success) {
                try {
                    await me.setNickname(finalNickname);
                    success = true;
                    successCount++;
                    console.log(`✅ [${guild.name}] Nickname changed to: "${finalNickname}"`);
                } catch (error) {
                    retryCount++;
                    if (retryCount >= 3) {
                        throw error; // Re-throw after 3 attempts
                    }
            ...
            failCount++;
            if (error.code === 50013) {
                console.error(`❌ [${guild.name}] Missing permissions - Drag bot role higher in role list`);
            } else if (error.code === 50035) {
                console.error(`❌ [${guild.name}] Invalid nickname format: ${error.message}`);
            } else {
                console.error(`❌ [${guild.name}] Failed to update nickname: ${error.message}`);
            }
        }
    }
    
    console.log(`📊 [GLOBAL] Nickname rotation complete: ${successCount} success, ${failCount} failed, ${permissionIssues} permission issues`);
    
    if (permissionIssues > 0) {
        console.log(`💡 [GLOBAL] FIX: Go to each server → Server Settings → Roles → Drag "${client.user.username}" role UP in the list`);
        console.log(`💡 [GLOBAL] The bot role must be ABOVE @everyone but can be below other roles`);
    }
    
    currentNicknameIndex = (currentNicknameIndex + 1) % nicknameStates.length;
    console.log(`⏭️ [GLOBAL] Next rotation will use: "SERVER_NAME | ${nextStatusSuffix}" in 20 seconds`);
}
...
client.once(Events.ClientReady, async () => {
    ...
    // Start bot nickname rotation every 20 seconds
    await updateBotNicknames(); // Initial update
    setInterval(updateBotNicknames, 20000);
});
...
client.on('interactionCreate', async (interaction) => {
    ...
    try {
        ...
        switch (interaction.customId) {
            case 'whitelist_this_bot':
                const botId = client.user.id;
                if (data.whitelistedBots.has(botId)) {
                    await interaction.reply({ content: '✅ This bot is already whitelisted!', ephemeral: true });
                } else {
                    data.whitelistedBots.add(botId);
                    addActivityLog(interaction.guild.id, 'BOT_WHITELISTED', `Self-whitelisted via button`, interaction.user, client.user);
                    await updateUnifiedWidget(interaction.guild);
                    await interaction.reply({ content: '✅ **This bot has been whitelisted successfully!**', ephemeral: true });
                }
                break;
...
client.on('guildMemberAdd', async (member) => {
    ...
    try {
        ...
        // Check if bot is whitelisted or has a whitelisted bot role
        const hasWhitelistedRole = hasWhitelistedBotRole(member, data);
        
        if (!data.whitelistedBots.has(botId) && !hasWhitelistedRole) {
            ...
            // If bot has whitelisted bot role but isn't in whitelist, add it
            if (hasWhitelistedRole && !data.whitelistedBots.has(botId)) {
                data.whitelistedBots.add(botId);
                console.log(`✅ [${member.guild.name}] AUTO-WHITELISTED BOT (via role): ${member.user.tag}`);
                addActivityLog(member.guild.id, 'BOT_AUTO_WHITELISTED', `Auto-whitelisted via bot role: ${member.user.tag}`, null, member.user);
            } else {
...
client.on('messageCreate', async (message) => {
    ...
    // Add bot whitelist command
    if (command === 'addbotw') {
        const botId = args[0];
        if (!botId || !/^\d{17,19}$/.test(botId)) {
            const reply = await message.reply('❌ Invalid bot ID. Usage: `!addbotw <bot_id>`');
            setTimeout(() => reply.delete().catch(() => {}), 3000);
            return;
        }
        ...
        try {
            ...
            data.whitelistedBots.add(botId);
            const reply = await message.reply(`✅ **WHITELISTED BOT:** ${bot.tag}`);
    ...
    // Add user role whitelist command
    if (command === 'addrolew') {
        const roleNameOrMention = args.join(' ').trim();
        ...
        try {
            ...
            if (!role) {
                const reply = await message.reply('❌ Role not found. Usage: `!addrolew <@role>` or `!addrolew <role_id>`');
                setTimeout(() => reply.delete().catch(() => {}), 3000);
                return;
            }
            ...
            const reply = await message.reply(`✅ **WHITELISTED USER ROLE:** "${role.name}"`);
    ...
    // Add bot role whitelist command
    if (command === 'addrolewbot') {
        const roleNameOrMention = args.join(' ').trim();
        const roleId = roleNameOrMention?.replace(/[<@&>]/g, '');
        ...
        try {
            ...
            if (!role) {
                const reply = await message.reply('❌ Role not found. Usage: `!addrolewbot <@role>` or `!addrolewbot <role_id>`');
                setTimeout(() => reply.delete().catch(() => {}), 3000);
                return;
            }

            if (data.whitelistedBotRoles.has(role.id)) {
                const reply = await message.reply(`✅ Bot role "${role.name}" is already whitelisted.`);
                setTimeout(() => reply.delete().catch(() => {}), 3000);
                return;
            }

            data.whitelistedBotRoles.add(role.id);
            
            // Auto-whitelist all bots with this role
            const botsWithRole = message.guild.members.cache.filter(m => m.user.bot && m.roles.cache.has(role.id));
            let whitelistedCount = 0;
            for (const [botId, member] of botsWithRole) {
                if (!data.whitelistedBots.has(botId)) {
                    data.whitelistedBots.add(botId);
                    whitelistedCount++;
                }
            ...
            const reply = await message.reply(`✅ **WHITELISTED BOT ROLE:** "${role.name}"\n🤖 Auto-whitelisted ${whitelistedCount} bot(s) with this role.`);
            addActivityLog(message.guild.id, 'BOT_ROLE_WHITELISTED', `Whitelisted bot role: ${role.name}, auto-whitelisted ${whitelistedCount} bots`, message.author, null);
            await updateUnifiedWidget(message.guild);
            setTimeout(() => reply.delete().catch(() => {}), 5000);
        } catch (error) {
            console.error('Error adding whitelisted bot role:', error);
            const reply = await message.reply('❌ Error whitelisting bot role.');
            setTimeout(() => reply.delete().catch(() => {}), 3000);
        }
    }

    // Show whitelisted roles command
    if (command === 'showwrole') {
        const userRoles = [];
        const botRoles = [];
        
        for (const roleId of data.whitelistedRoleIds) {
            const role = message.guild.roles.cache.get(roleId);
            if (role) userRoles.push(`• ${role.name} (${roleId})`);
        }
        
        for (const roleId of data.whitelistedBotRoles) {
            const role = message.guild.roles.cache.get(roleId);
            if (role) botRoles.push(`• ${role.name} (${roleId})`);
        }
        
        const embed = new EmbedBuilder()
            .setTitle('🛡️ WHITELISTED ROLES')
            .addFields(
                { name: '👤 User Roles', value: userRoles.length > 0 ? userRoles.join('\n') : 'None', inline: false },
                { name: '🤖 Bot Roles', value: botRoles.length > 0 ? botRoles.join('\n') : 'None', inline: false }
            )
            .setColor(0x00FF00)
            .setTimestamp();
    ...
    // Show whitelisted bots command
    if (command === 'showwbot') {
        const botList = [];
        
        for (const botId of data.whitelistedBots) {
            const bot = await client.users.fetch(botId).catch(() => null);
            if (bot) {
                botList.push(`• ${bot.tag} (${botId})`);
            } else {
                botList.push(`• Unknown Bot (${botId})`);
            }
        }
        
        const embed = new EmbedBuilder()
            .setTitle('🤖 WHITELISTED BOTS')
            .setDescription(botList.length > 0 ? botList.join('\n') : 'No bots whitelisted')
            .setColor(0x00FF00)
            .setFooter({ text: `Total: ${data.whitelistedBots.size} bot(s)` })
            .setTimestamp();
        
        await message.reply({ embeds: [embed] });
    }

    // Whitelist all bots command
    if (command === 'wbotall') {
        const allBots = message.guild.members.cache.filter(member => member.user.bot);
        let newlyWhitelisted = 0;
        let alreadyWhitelisted = 0;
        
        for (const [botId, botMember] of allBots) {
            if (data.whitelistedBots.has(botId)) {
                alreadyWhitelisted++;
            } else {
                data.whitelistedBots.add(botId);
        ...
        const embed = new EmbedBuilder()
            .setTitle('✅ MASS BOT WHITELIST')
            .setDescription(`**Successfully whitelisted all bots in the server!**`)
            .addFields(
                { name: '🆕 Newly Whitelisted', value: newlyWhitelisted.toString(), inline: true },
                { name: '✅ Already Whitelisted', value: alreadyWhitelisted.toString(), inline: true },
                { name: '📊 Total Bots', value: allBots.size.toString(), inline: true }
            )
            .setColor(0x00FF00)
            .setTimestamp();
        ...
        addActivityLog(message.guild.id, 'MASS_BOT_WHITELIST', `Whitelisted all bots: ${newlyWhitelisted} new, ${alreadyWhitelisted} existing`, message.author, null);
        await updateUnifiedWidget(message.guild);
        
        console.log(`✅ [${message.guild.name}] Mass whitelisted all bots: ${newlyWhitelisted} new, ${alreadyWhitelisted} already whitelisted`);
    }
    ...
    if (command === 'release') {
        ...
        if (!userId) {
            const reply = await message.reply('❌ Usage: `!release <@user>` or `!release <user_id>`');
            setTimeout(() => reply.delete().catch(() => {}), 3000);
            return;
        }
    ...
    if (command === 'help') {
        const helpEmbed = new EmbedBuilder()
            ...
            .setColor(0x0099FF)
            .addFields(
                { name: '🔧 OWNER COMMANDS', value: '`!24/7` - Toggle monitoring\n`!addbotw <bot_id>` - Whitelist bot\n`!wbotall` - Whitelist all bots\n`!addrolew <@role>` - Whitelist user role\n`!addrolewbot <@role>` - Whitelist bot role\n`!showwrole` - Show whitelisted roles\n`!showwbot` - Show whitelisted bots\n`!release <user>` - Release timeout\n`!joinvc` - Join voice channel\n`!cleanup` - Clean channel', inline: false },
                ...
                { name: '🤖 BOT PROTECTION', value: '**WHITELIST ONLY:** Only whitelisted bots allowed\n**INSTANT KICK:** Unauthorized bots removed\n**PERMANENT BAN:** Users adding unauthorized bots\n**BOT ROLES:** Bots with whitelisted roles auto-whitelisted', inline: false },
                ...
                { name: '🔘 WIDGET BUTTONS', value: '**Whitelist This Bot:** 1-click to whitelist the bot in the server', inline: false }
            )
            .setTimestamp();
